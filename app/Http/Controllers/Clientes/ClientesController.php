...
class ClientesController extends Controller
{
    ...
    {
        $cliente = DB::table('Clientes')->where('Codcli', $id)->first();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $cliente_id = DB::table('Clientes')->insertGetId([
                'Codcli' => $this->generarCodigoCliente(),
                ...
                'Credit_limit' => $request->limite_credito ?? 0,
            ...
            if ($request->vendedor_asignado) {
                DB::table('clientes_vendedores')->insert([
                    'cliente_id' => $cliente_id,
                    'vendedor_id' => $request->vendedor_asignado,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Actualizar cliente
            DB::table('Clientes')->where('Codcli', $id)->update([
                'Razonsocial' => $request->razon_social,
                'Ruc' => $request->ruc,
                'Direccion' => $request->direccion,
                'Distrito' => $request->distrito,
                ...
                'Credit_limit' => $request->limite_credito,
            ...
            if (isset($request->vendedor_asignado)) {
                DB::table('clientes_vendedores')
                    ->where('cliente_id', $id)
                    ->delete();
    ...
    public function analizarCredito($cliente_id)
    {
        ...
        $credito_disponible = $cliente->Credit_limit - $saldo_actual;
        $porcentaje_utilizado = $cliente->Credit_limit > 0 ? ($saldo_actual / $cliente->Credit_limit) * 100 : 0;
    ...
    public function porVendedor($vendedor_id)
    {
        $clientes = DB::table('Clientes')
            ->join('clientes_vendedores', 'Clientes.Codcli', '=', 'clientes_vendedores.cliente_id')
            ->where('clientes_vendedores.vendedor_id', $vendedor_id)
            ->where('Clientes.Estado', 'ACTIVO')
            ->select(
                'Clientes.*',
                'clientes_vendedores.fecha_asignacion'
            )
            ->get();
    ...
    public function buscar(Request $request)
    {
        ...
        if ($request->busqueda) {
            $query->where(function($q) use ($request) {
                $q->where('Clientes.Razonsocial', 'like', '%' . $request->busqueda . '%')
                  ->orWhere('Clientes.Ruc', 'like', '%' . $request->busqueda . '%')
                  ->orWhere('Clientes.Codcli', 'like', '%' . $request->busqueda . '%');
            });
        }
        ...
        $clientes = $query->orderBy('Clientes.Razonsocial')->paginate($request->per_page ?? 20);
    ...
    public function generarRecomendaciones($cliente_id)
    {
        ...
        $compras_recientes = DB::table('Doccab')
            ...
            ->where('Doccab.Codcli', $cliente_id)
    ...
    public function obtenerActividadReciente($cliente_id)
    {
        ...
        $actividades_sistema = DB::table('actividad_clientes')
    ...
    public function generarCodigoCliente()
    {
        ...
        $numero = $ultimo ? (int) substr($ultimo->Codcli, 2) + 1 : 1;
    ...
    {
        DB::table('actividad_clientes')->insert([
            'cliente_id' => $cliente_id,
            'accion' => $accion,
    ...
    public function reporteClientesVip()
    {
        return DB::table('Doccab')
            ...
            ->where('Doccab.Fecha', '>=', now()->subYear())
            ->groupBy('Clientes.Codcli', 'Clientes.Razonsocial', 'Clientes.Ruc', 'Clientes.Telefono', 'Clientes.Email')
            ->orderBy('total_compras', 'desc')
            ->get();
    }
